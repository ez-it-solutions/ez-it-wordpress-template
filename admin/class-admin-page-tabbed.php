...
class EzIT_Template_Admin_Page_Tabbed {
    ...
    public static function render() {
        ...
        <div class="ezit-fullpage <?php echo esc_attr($theme_class); ?>" id="ezit-main-wrap" data-theme="<?php echo esc_attr($current_theme); ?>">
            ...
            <!-- Tab Navigation -->
            <nav class="ezit-tabs">
                <a href="?page=ez-it-template&tab=dashboard" class="ezit-tab <?php echo $current_tab === 'dashboard' ? 'active' : ''; ?>" data-tab="dashboard">
                    <span class="dashicons dashicons-dashboard"></span>
                    Dashboard
                </a>
                <a href="?page=ez-it-template&tab=settings" class="ezit-tab <?php echo $current_tab === 'settings' ? 'active' : ''; ?>" data-tab="settings">
                    <span class="dashicons dashicons-admin-settings"></span>
                    Settings
                </a>
                <a href="?page=ez-it-template&tab=about" class="ezit-tab <?php echo $current_tab === 'about' ? 'active' : ''; ?>" data-tab="about">
            ...
            <div class="ezit-content-wrapper">
                <!-- Loading Modal -->
                <div class="ezit-loading-modal" id="ezit-loading-modal">
                    <div class="ezit-loading-content">
                        <div class="ezit-loading-spinner"></div>
                        <p class="ezit-loading-text">Loading...</p>
                    </div>
                </div>
    ...
    private static function render_notices($notice, $last_urls) {
        ...
        if (isset($notices[$notice])) {
            $n = $notices[$notice];
            $class = $n['type'] === 'error' ? 'ezit-notice-error' : 'ezit-notice-success';
            echo '<div class="ezit-notice ' . $class . '">';
            echo '<p>' . $n['message'] . '</p>';
            
            if ($notice === 'tokens' && !empty($last_urls)) {
                foreach ($last_urls as $url) {
                    echo '<p><input class="ezit-copy" type="text" readonly value="' . esc_attr($url) . '"></p>';
                }
            }
            echo '</div>';
        }
    ...
    public static function render_dashboard_tab($dh_status) {
        ?>
        <h2 class="ezit-section-title">Welcome to Ez DH SSO Utility</h2>
        <p class="ezit-intro">Manage DreamHost SSO, generate login tokens, impersonate users, and migrate WordPress sites with ease.</p>
        
        <div class="ezit-dashboard-grid">
            <div class="ezit-card">
                <div class="ezit-card-icon ezit-icon-success">
                    <span class="dashicons dashicons-admin-plugins"></span>
                </div>
                <h3>DreamHost Plugin</h3>
                <p class="ezit-status-item">
                    <span class="ezit-label">Status:</span>
                    <span class="ezit-badge <?php echo $dh_status['active'] ? 'ezit-badge-success' : 'ezit-badge-warning'; ?>">
                        <?php echo $dh_status['active'] ? 'Active' : 'Inactive'; ?>
                    </span>
                </p>
                <p class="ezit-status-item">
                    <span class="ezit-label">Installed:</span>
            ...
            <div class="ezit-card">
                <div class="ezit-card-icon ezit-icon-primary">
            ...
            <div class="ezit-card">
                <div class="ezit-card-icon ezit-icon-info">
        ...
        <div class="ezit-quick-stats">
            <h3>Quick Stats</h3>
            <div class="ezit-stats-grid">
                <div class="ezit-stat">
                    <span class="ezit-stat-value"><?php echo wp_count_posts('page')->publish; ?></span>
                    <span class="ezit-stat-label">Pages</span>
                </div>
                <div class="ezit-stat">
                    <span class="ezit-stat-value"><?php echo count_users()['total_users']; ?></span>
                    <span class="ezit-stat-label">Users</span>
                </div>
                <div class="ezit-stat">
                    <span class="ezit-stat-value"><?php echo is_plugin_active('dreamhost-panel-login/dreamhost-panel-login.php') ? 'Active' : 'Inactive'; ?></span>
                    <span class="ezit-stat-label">DH Plugin</span>
                </div>
            </div>
        </div>
        <?php
    ...
    public static function render_sso_tab($dh_status, $unhide, $roles, $selected_role, $users) {
        ?>
        <h2 class="ezit-section-title">SSO & Token Management</h2>
        
        <div class="ezit-two-column">
            <div class="ezit-card">
                <h3><span class="dashicons dashicons-admin-plugins"></span> DreamHost Plugin Status</h3>
                <ul class="ezit-status-list">
        ...
        <div class="ezit-card">
            ...
            <ul class="ezit-status-list">
                ...
                <?php if ($last_token_truncated): ?>
                    <li><strong>Last token (truncated):</strong> <code class="ezit-code"><?php echo esc_html($last_token_truncated); ?>...</code></li>
                <?php endif; ?>
                <?php if ($last_token): ?>
                ...
                <li><strong>SSO endpoint:</strong> <code class="ezit-code"><?php echo esc_html(home_url('/wp-login.php')); ?></code></li>
            </ul>
            
            <?php if ($last_token): ?>
                <div class="ezit-token-display">
                    <h4 style="margin: 0 0 8px 0; font-size: 0.9rem;">Full Token (for testing):</h4>
                    <textarea readonly class="ezit-token-textarea" onclick="this.select();"><?php echo esc_textarea($last_token); ?></textarea>
    ...
    public static function render_impersonation_tab($roles, $selected_role, $users) {
        ?>
        <h2 class="ezit-section-title">User Impersonation</h2>
        <p class="ezit-intro">Safely login as any user to test their experience. You can return to your admin account at any time.</p>
    ...
    public static function render_migration_tab() {
        ...
        ?>
        <h2 class="ezit-section-title">DreamHost Migration Tools</h2>
        <p class="ezit-intro">Connect to external WordPress databases to update site URLs and generate login tokens during migrations.</p>
        ...
        <div class="ezit-two-column">
            ...
            <div class="ezit-card">
                ...
                <?php if ($db_connected): ?>
                    <p class="ezit-success-message">✓ Connected to: <?php echo esc_html($db_name); ?></p>
                ...
                <?php else: ?>
                    <p class="ezit-warning-message">Please test and save your database connection first.</p>
                <?php endif; ?>
            </div>
        </div>
        <?php
    ...
    public static function render_settings_tab($current_theme) {
        ...
        ?>
        <h2 class="ezit-section-title">Plugin Settings</h2>
        
        <?php if (isset($_GET['ez_notice']) && $_GET['ez_notice'] === 'settings_saved'): ?>
            <div class="ezit-success-message" style="padding: 12px 16px; background: rgba(34, 197, 94, 0.1); border-left: 3px solid #22c55e; border-radius: 4px; margin-bottom: 20px;">
        ...
        <div class="ezit-card">
            ...
            <div class="ezit-theme-selector">
                <div class="ezit-theme-option <?php echo $current_theme === 'light' ? 'active' : ''; ?>" data-theme="light">
                    <div class="ezit-theme-preview ezit-theme-preview-light">
                        <div class="ezit-preview-header"></div>
                        <div class="ezit-preview-content">
                            <div class="ezit-preview-card"></div>
                            <div class="ezit-preview-card"></div>
                        </div>
                    </div>
                    <div class="ezit-theme-info">
                        <h4>☀️ Light Mode</h4>
                        <p>Clean and bright, ideal for daytime use</p>
                    </div>
                    <?php if ($current_theme === 'light'): ?>
                        <span class="ezit-theme-active-badge">Active</span>
                    <?php endif; ?>
                </div>
                
                <div class="ezit-theme-option <?php echo $current_theme === 'dark' ? 'active' : ''; ?>" data-theme="dark">
                    <div class="ezit-theme-preview ezit-theme-preview-dark">
                        <div class="ezit-preview-header"></div>
                        <div class="ezit-preview-content">
                            <div class="ezit-preview-card"></div>
                            <div class="ezit-preview-card"></div>
        ...
        <div class="ezit-card">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" id="ezit-advanced-settings-form">
                <?php wp_nonce_field('ez_dh_save_settings'); ?>
                <input type="hidden" name="action" value="ez_dh_save_settings">
                
                <div class="ezit-settings-grid">
                    <div class="ezit-setting-item">
                        ...
                        <p class="ezit-setting-description">Cache plugin data to improve performance. Recommended for production sites.</p>
                    </div>
                    ...
                    <div class="ezit-setting-item">
                        ...
                        <p class="ezit-setting-description">Enable detailed logging for troubleshooting. Disable in production.</p>
                    </div>
                    ...
                    <div class="ezit-setting-item">
                        ...
                        <p class="ezit-setting-description">Automatically delete expired tokens to keep database clean.</p>
                    </div>
                    ...
                    <div class="ezit-setting-item">
                        ...
                        <p class="ezit-setting-description">How long tokens remain valid before expiring (1-168 hours).</p>
                    </div>
                </div>
    ...
    public static function render_about_tab() {
        ...
        ?>
        <h2 class="ezit-section-title">About Ez DH SSO Fix</h2>
        ...
        <div class="ezit-two-column">
            <div class="ezit-card">
                ...
                <ul class="ezit-status-list">
                    ...
                    <li><strong>Website:</strong> <a href="https://www.Ez-IT-Solutions.com" target="_blank" class="ezit-website-link">www.Ez-IT-Solutions.com</a></li>
        ...
        <div class="ezit-card">
            ...
            <div class="ezit-two-column" style="margin-top: 15px;">
                <div>
    ...
    public static function render_sidebar($current_tab) {
        ...
        <div class="ezit-sidebar-card ezit-promo">
